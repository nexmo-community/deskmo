...
if (typeof CONVERSATION_ID !== "undefined" && CONVERSATION_ID !== "") {
    ...
    new ConversationClient({ debug: false }).login(USER_JWT).then(function (app) {
        app.getConversation(CONVERSATION_ID).then(function (conversation) {
            ...
            conversation.on("text:typing:on", function (data) {
                typingIndicator.text(data.user.name + " is typing...");
                replyInput.after(typingIndicator);
            });
            conversation.on("text:typing:off", function (data) {
